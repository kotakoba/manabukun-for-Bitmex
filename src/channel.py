...
class ChannelBreakOut:
    ...
    def executionsProcess(self):
        ...
        resolution = 5
        now = str(int(time.time())) #unixtime
        #print('https://www.bitmex.com/api/udf/history?symbol=XBTUSD&resolution='+str(resolution)+'&from='+ str(int(now)-60*5*30) + '&to=' +now)
        r = requests.get('https://www.bitmex.com/api/udf/history?symbol=XBTUSD&resolution='+str(resolution)+'&from='+ str(int(now)-60*5*30) + '&to=' +now) #get ohlcv
        self.executions = r.json()#change form to json
        #print(max(ohlcv['h']))
        """
